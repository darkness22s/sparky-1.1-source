...
use sparky_config::{
    max_output_tokens_for_model, DEFAULT_CONTEXT_WINDOW_TOKENS, DEFAULT_MAX_TURNS,
};
...
mod tests {
    ...
    #[async_trait]
    impl LlmProvider for ToolLimitProvider {
        fn provider_name(&self) -> &str {
            "test"
        }
        ...
        async fn stream(
            ...
            if call == 0 {
                ...
                return Ok(Box::pin(stream::iter(vec![
                    AssistantMessageEvent::TextDelta("Let me inspect the file.".to_string()),
                    AssistantMessageEvent::ToolCallDelta {
                        id: "call-1".to_string(),
                        name: "read".to_string(),
            ...
            Ok(Box::pin(stream::iter(vec![
                AssistantMessageEvent::TextDelta(
                    "I inspected the file and saved the completed work.".to_string(),
                ),
                AssistantMessageEvent::Done { usage: None },
            ])))
        }
    ...
    #[tokio::test]
    async fn forces_a_closing_response_after_the_tool_turn_limit() {
        let workspace = temporary_workspace();
        tokio::fs::create_dir_all(&workspace).await.unwrap();
        tokio::fs::write(workspace.join("README.md"), "test content\n")
            .await
            .unwrap();
        let cwd = workspace.to_string_lossy().to_string();
        let mut session = SessionManager::create_new(&cwd, None).await.unwrap();
        let provider = Arc::new(ToolLimitProvider {
            calls: std::sync::atomic::AtomicUsize::new(0),
        });
        let agent = AgentLoop::new(
            provider.clone(),
        ...
        let response = agent
            .run_turn_streaming("Inspect the file", &mut session, |event| {
                if let AssistantMessageEvent::TextDelta(delta) = event {
                    deltas.push(delta.clone());
                }
            })
        ...
        assert_eq!(
            ...
            vec![
                ...
                "I inspected the file and saved the completed work."
            ]
        );
        assert_eq!(
            response,
            "Let me inspect the file.I inspected the file and saved the completed work."
        );
        remove_temporary_workspace(workspace).await;
    }
...
impl AgentLoop {
        ...
        F: FnMut(&AssistantMessageEvent),
    {
        ...
        while turn_count < self.options.max_turns {
            turn_count += 1;
            ...
            if let Some(context_window) = self.options.context_window_tokens {
                if let Err(error) = self
                    ...
                    .await
                {
                    tracing::warn!(%error, "context compaction failed; continuing with bounded session context");
                }
            }
        ...
        if !completed_naturally {
            tracing::warn!(
                "Agent loop reached maximum turns limit ({}); requesting a final response without tools",
            ...
            let mut context_messages = vec![Message::system(&system_prompt)];
            context_messages.extend(session.build_context_messages());
            context_messages.push(Message::user(
                "You have reached the tool-call limit. Do not call any more tools. Give the user a concise final response now. Summarize what you completed, what remains, and the current verification status. Never end with an unfinished sentence or a promise to perform another action.",
            ));
            let completion_options = CompletionOptions {
                model: self.options.model_name.clone(),
                temperature: self.options.temperature,
            ...
            while let Some(event) = event_stream.next().await {
                on_event(&event);
                match event {
                    AssistantMessageEvent::TextDelta(delta) => closing_response.push_str(&delta),
                    AssistantMessageEvent::Error(message) => anyhow::bail!(message),
                    AssistantMessageEvent::Done { usage: Some(usage) } => {
                        self.record_usage(&mut cumulative_usage, &usage);
                    }
            ...
            if closing_response.trim().is_empty() {
                closing_response = "I reached the tool-call limit before I could finish. The completed changes are saved, but additional work or verification may still be needed.".to_string();
