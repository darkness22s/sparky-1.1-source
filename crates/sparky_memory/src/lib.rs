...
impl MemoryStore {
    pub async fn load(cwd: &str, global_dir: Option<&Path>) -> anyhow::Result<Self> {
        ...
        let global_file = global_dir
            ...
            .unwrap_or_else(|| Path::new(cwd).join(".sparky").join("global").join(MEMORY_FILE_NAME));
        let mut memories = read_file(&global_file).await?;
        memories.extend(read_file(&project_file).await?);
        Ok(Self { global_file, project_file, memories })
    }

    pub fn all(&self) -> &[Memory] { &self.memories }

    pub fn search(&self, query: &str, limit: usize) -> Vec<Memory> {
        let terms: Vec<String> = query.split_whitespace().map(|term| term.to_ascii_lowercase()).collect();
        let mut matches: Vec<(usize, Memory)> = self.memories.iter().filter_map(|memory| {
            let haystack = format!("{} {} {}", memory.title, memory.content, memory.category).to_ascii_lowercase();
            let score = terms.iter().filter(|term| haystack.contains(term.as_str())).count();
            (score > 0 || terms.is_empty()).then_some((score, memory.clone()))
        }).collect();
        matches.sort_by(|a, b| b.0.cmp(&a.0).then_with(|| b.1.importance.cmp(&a.1.importance)));
        matches.into_iter().take(limit).map(|(_, memory)| memory).collect()
    }
    ...
    pub async fn upsert(&mut self, mut memory: Memory) -> anyhow::Result<()> {
        ...
        memory.category = if memory.category.trim().is_empty() { "general".into() } else { memory.category.trim().into() };
        memory.importance = memory.importance.clamp(1, 5);
        memory.updated_at = Utc::now().to_rfc3339();
        if memory.created_at.is_empty() { memory.created_at = memory.updated_at.clone(); }
    ...
    pub async fn add(&mut self, scope: MemoryScope, title: String, content: String, category: String, importance: u8, source: Option<String>) -> anyhow::Result<Memory> {
        let now = Utc::now().to_rfc3339();
        let memory = Memory { id: Uuid::new_v4().to_string(), scope, title, content, category, importance, created_at: now.clone(), updated_at: now, source };
    ...
    pub async fn delete(&mut self, id: &str) -> anyhow::Result<bool> {
        ...
        if before == self.memories.len() { return Ok(false); }
        self.persist().await?;
        Ok(true)
    }
    ...
    pub fn render_context(&self, query: &str) -> String {
        ...
        for memory in memories {
            ...
            if output.chars().count() + entry.chars().count() > MAX_INJECTED_MEMORY_CHARS { break; }
    ...
    async fn persist(&self) -> anyhow::Result<()> {
        let global: Vec<_> = self.memories.iter().filter(|memory| memory.scope == MemoryScope::Global).cloned().collect();
        let project: Vec<_> = self.memories.iter().filter(|memory| memory.scope == MemoryScope::Project).cloned().collect();
...
async fn read_file(path: &Path) -> anyhow::Result<Vec<Memory>> {
    let Ok(raw) = fs::read_to_string(path).await else { return Ok(Vec::new()); };
...
async fn write_file(path: &Path, memories: &[Memory]) -> anyhow::Result<()> {
    ...
    let file = MemoryFile { version: 1, memories: memories.to_vec() };
...
mod tests {
    ...
    async fn persists_scoped_memories_and_renders_relevant_context() {
        ...
        let mut store = MemoryStore::load(dir.path().to_str().unwrap(), Some(global.path())).await.unwrap();
        store.add(MemoryScope::Project, "Test command".into(), "Run cargo test -p sparky_memory".into(), "workflow".into(), 5, None).await.unwrap();
        store.add(MemoryScope::Global, "Preferred style".into(), "Keep changes small".into(), "preference".into(), 4, None).await.unwrap();
        let reloaded = MemoryStore::load(dir.path().to_str().unwrap(), Some(global.path())).await.unwrap();
        assert_eq!(reloaded.all().len(), 2);
        assert!(reloaded.render_context("cargo test").contains("Test command"));
    }

    #[tokio::test]
    async fn rejects_secrets() {
        let dir = tempdir().unwrap();
        let mut store = MemoryStore::load(dir.path().to_str().unwrap(), None).await.unwrap();
        let error = store.add(MemoryScope::Project, "Key".into(), "api_key=secret".into(), "general".into(), 3, None).await.unwrap_err();
