...
/// messages to disk. Also checks whether any of these dirs are already tracked
/// by git and removes them from the index via `git rm --cached` so the
/// gitignore entry actually takes effect.
async fn ensure_session_dirs_gitignored(cwd: &str) -> anyhow::Result<()> {
    ...
    // Remove any already-tracked sparky session files from git's index.
    // .gitignore only affects untracked files; tracked files need explicit
    // removal from the index to stop showing up in diffs.
    let sparky_path = Path::new(cwd).join(".sparky");
    if sparky_path.exists() {
        let _ = Command::new("git")
            .args(["rm", "--cached", "-r", "--quiet", ".sparky/"])
            .current_dir(cwd)
            .output()
            .await;
    }
