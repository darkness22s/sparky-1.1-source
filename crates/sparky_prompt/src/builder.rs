...
impl PromptBuilder {
    ...
    pub async fn load_context_files(&self) -> Vec<ContextFile> {
        ...
        for (path, relative) in self.project_files() {
...
## Core behavior\n\
            \n\
* Continue working until the task is complete or genuinely blocked.\n\
...
**edit**  -  Replace an exact, unique text block in an existing file. This is the only tool for modifying existing project files. Keep `oldText` as short as possible while remaining unique. Break complex changes into multiple targeted edits.
...
",
        ...
        let ctx_files = self.load_context_files().await;
