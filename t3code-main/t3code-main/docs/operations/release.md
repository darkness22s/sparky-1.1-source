...
- Reads the shared production Sparky Cloud relay URL and Clerk client configuration before packaging clients.
...
- Provider: the public Convex-backed update feed configured at build time with `SPARKY_UPDATE_URL`.
- The feed must be the architecture-specific `/updates/...` route for the target build. It redirects
  to the published Electron metadata after Convex verifies the currently published release.
...
10. Complete the Clerk Native API and AASA setup in [Sparky Cloud Clerk Setup](../cloud/t3-connect-clerk.md#desktop-passkeys).
