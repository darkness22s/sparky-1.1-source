...
describe("DesktopSavedEnvironments", () => {
  ...
  it.effect("reports the failed saved environment write operation and path", () =>
    Effect.gen(function* () {
      ...
      const registryDirectory = NodePath.join(baseDir, "userdata");
      const permissionError = PlatformError.systemError({
        _tag: "PermissionDenied",
        module: "FileSystem",
        method: "makeDirectory",
        pathOrDescriptor: registryDirectory,
      });
      const fileSystemLayer = Layer.succeed(
        FileSystem.FileSystem,
        FileSystem.makeNoop({
      ...
      assert.equal(error.path, registryDirectory);
      assert.strictEqual(error.cause, permissionError);
      assert.equal(
        error.message,
        `Desktop saved-environment write failed during create-directory at ${registryDirectory}.`,
      );
      assert.notEqual(error.message, permissionError.message);
    }).pipe(Effect.provide(NodeServices.layer), Effect.scoped),
  );
