...
describe("getDesktopUpdateActionError", () => {
  it("returns user-visible message for accepted failed attempts", () => {
    ...
    expect(getDesktopUpdateActionError(result)).toBe(
      "Sparky couldn't download this update. Try again in a moment.",
    );
  });
...
describe("desktop update UI helpers", () => {
  ...
  it("includes the downloaded version in the install confirmation copy", () => {
    ...
    ).toContain("Install update 1.1.1 and restart Sparky?");
  });
  ...
  it("falls back to generic install confirmation copy when no version is available", () => {
    ...
    ).toContain("Install update and restart Sparky?");
  });
});
