...
function OpenCommandPaletteDialog(props: {
  ...
  const buildAddProjectSourceGroups = useCallback(
    (
      ...
      _readinessBySource: AddProjectRemoteSourceReadiness,
      ...
      const sourceItems: Array<CommandPaletteActionItem | CommandPaletteSubmenuItem> = [
        ...
        {
          kind: "action",
          value: `action:add-project:${environmentId}:url`,
          searchTerms: ["clone", "remote", "repository", "repo", "git", "url"],
          title: "Git URL",
          description: "Clone from a remote URL",
          icon: <GitBranchIcon className={ITEM_ICON_CLASS} />,
          keepOpen: true,
          run: async () => {
            startAddProjectClone(environmentId, "url");
          },
        },
      ];
  ...
  const startAddProjectSourceSelection = useCallback(
    (environmentId: EnvironmentId): void => {
      ...
      pushPaletteView({
        addonIcon: <FolderPlusIcon className={ADDON_ICON_CLASS} />,
        groups: buildAddProjectSourceGroups(environmentId, {} as AddProjectRemoteSourceReadiness),
      });
    },
