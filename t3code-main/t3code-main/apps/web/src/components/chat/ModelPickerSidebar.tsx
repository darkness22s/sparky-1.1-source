...
export type ModelCatalogSectionId = `catalog:${string}`;

export type ModelCatalogSection = {
  readonly id: ModelCatalogSectionId;
  readonly label: string;
  readonly Icon: Icon;
};

export type ModelPickerRailSelection = ProviderInstanceId | "favorites" | ModelCatalogSectionId;
...
export const ModelPickerSidebar = memo(function ModelPickerSidebar(props: {
  ...
  /** Virtual catalogs that still route through the real Sparky instance. */
  catalogSections?: ReadonlyArray<ModelCatalogSection>;
  ...
  }, [props.catalogSections, props.instanceEntries, props.selectedInstanceId, showFavorites]);
    ...
    >
      <div className="h-full overflow-y-auto overscroll-contain [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        >
          ...
          {props.catalogSections?.map((section) => {
            const isSelected = props.selectedInstanceId === section.id;
            return (
              <div key={section.id} className="relative w-full">
                <Tooltip>
                  <TooltipTrigger
                    render={
                      <button
                        data-model-picker-provider={section.id}
                        ...
                        onMouseEnter={() => setHoveredInstanceId(section.id)}
                        onMouseLeave={() =>
                      ...
                      >
                        <section.Icon
                          className={cn(
                            "size-5 shrink-0",
                            isSelected || hoveredInstanceId === section.id
                              ? "text-foreground"
