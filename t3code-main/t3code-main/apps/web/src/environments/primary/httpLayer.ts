...
/**
 * Fetch a raw primary-environment route with the same authentication and
 * credential policy as the Effect HTTP client used by the rest of the app.
 */
export async function fetchPrimaryEnvironment(
  input: RequestInfo | URL,
  init: RequestInit = {},
): Promise<Response> {
  const sameOriginBrowserPrimary = isSameOriginBrowserPrimary();
  const headers = new Headers(init.headers);

  if (!sameOriginBrowserPrimary && !headers.has("authorization")) {
  ...
  return globalThis.fetch(input, {
    ...init,
    headers,
    credentials: sameOriginBrowserPrimary ? "include" : "omit",
  });
}
