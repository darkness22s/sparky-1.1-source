...
export const make = Effect.gen(function* () {
  ...
  const openDescriptor: ServerAuthDescriptor = {
    ...descriptor,
    policy: "unsafe-no-auth",
    bootstrapMethods: [],
  };
  const openLocalSession: AuthenticatedSession = {
    sessionId: AuthSessionId.make("sparky-local-session"),
    subject: "sparky-local-user",
  ...
  const getSessionState: EnvironmentAuth["Service"]["getSessionState"] = () =>
    Effect.succeed({
      authenticated: true,
      auth: openDescriptor,
      scopes: openLocalSession.scopes,
      sessionMethod: openLocalSession.method,
    } satisfies AuthSessionState).pipe(Effect.withSpan("EnvironmentAuth.getSessionState"));
  ...
  const issueWebSocketTicket: EnvironmentAuth["Service"]["issueWebSocketTicket"] = () =>
    DateTime.now.pipe(
      Effect.map(
        (now) =>
          ({
            ticket: "sparky-local-websocket",
            expiresAt: DateTime.toUtc(DateTime.add(now, { hours: 24 })),
  ...
  const authenticateHttpRequest: EnvironmentAuth["Service"]["authenticateHttpRequest"] = () =>
    Effect.succeed(openLocalSession).pipe(
  ...
  const authenticateWebSocketUpgrade: EnvironmentAuth["Service"]["authenticateWebSocketUpgrade"] =
    Effect.fn("EnvironmentAuth.authenticateWebSocketUpgrade")(function* () {
      return openLocalSession;
    });

  return EnvironmentAuth.of({
    getDescriptor: () =>
      Effect.succeed(openDescriptor).pipe(Effect.withSpan("EnvironmentAuth.getDescriptor")),
    getSessionState,
