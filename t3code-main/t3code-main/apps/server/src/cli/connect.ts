...
const authorizeCli = Effect.fn("cloud.cli.authorize")(function* (options: {
  ...
  const existing = yield* tokens.getExisting.pipe(
    Effect.catchTag("CloudCliCredentialRefreshError", () =>
      Console.log(
        "The stored Sparky Cloud credential could not be refreshed; signing in again.",
...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `t3 connect link` to authorize and enable Sparky Cloud."
    : !status.desired
      ? "Run `t3 connect link` to enable Sparky Cloud."
      : !status.linked
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The T3 relay client is required for Sparky Cloud. Download and install version ${version}?`,
      initial: false,
    }),
  );
...
const logCloudDisconnectFailure = (
  ...
  Effect.logWarning("Sparky Cloud disconnect operation failed.").pipe(
    Effect.annotateLogs({
      operation,
      clearAuthorization,
...
export const reportCloudDisconnectResults = Effect.fn("cloud.cli.report_disconnect_results")(
  function* (input: {
    ...
    if (input.liveResult.status === "failed") {
      ...
      yield* Console.warn(
        "Sparky Cloud is disabled, but the running server could not stop its tunnel.\nRestart that server to stop the connector.",
      );
    } else {
      yield* Console.log("Sparky Cloud is disabled locally.");
    }
...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (options.clearAuthorization) {
    ...
    yield* bootService.uninstall.pipe(
      Effect.tap((removed) =>
        removed ? Console.log("Removed the Sparky background service.") : Effect.void,
      ),
  ...
  if (options.clearAuthorization) {
    yield* Console.log("Signed out of Sparky Cloud locally.");
  }
});
...
const linkEnvironmentForConnect = Effect.fn("cloud.cli.link_environment")(function* (options: {
  ...
  if (!publishOnly) {
    ...
    if (Option.isNone(installed)) {
      yield* Console.log("Sparky Cloud setup cancelled. The relay client was not installed.");
      return null;
    }
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription("Authorize the Sparky Cloud CLI without enabling remote access."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log("Sparky Cloud\n");
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription("Authorize this environment for Sparky Cloud on next start."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription("Show persisted Sparky Cloud and relay client state."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log("Cancelled the pending publish-only Sparky Cloud link.");
          }
        ...
        // out of band without Sparky Cloud.
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A Sparky Cloud link is already pending. Start T3 to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription("Disable Sparky Cloud while retaining the stored authorization."),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription("Disable Sparky Cloud and clear the stored CLI authorization."),
  Command.withHandler((flags) =>
...
const offerBootService = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("Sparky is already set up to run in the background on this machine.");
    return true;
  }
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed Sparky background service is from an older setup. Update it now?"
        : "Run Sparky in the background whenever this machine boots? " +
          "It stays reachable through Sparky Cloud even after you log out.",
      initial: true,
    }),
  );
  if (!wanted) {
...
export const connectCommand = Command.make("connect", {
  ...
  Command.withDescription("Set up Sparky Cloud for this machine."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          background
            ? "\n✓ Background service ready\n\nSparky will stay reachable after you log out."
            : "\nNext\n  Start the server with `t3 serve` to make this machine reachable.",
        );
      }),
    ),
