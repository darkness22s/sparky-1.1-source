 ...
 *
 * Enforces the max-4-short-or-3-long-words rule by word-count clamping:
 * keeps the first 4 words, or 3 if any word exceeds 7 characters.
 */
export function sanitizeThreadTitle(raw: string): string {
  ...
  const maxWords = hasLongWord ? 3 : 4;
  const clamped = words.slice(0, maxWords).join(" ");

  if (clamped.length <= 50) {
    return clamped;
  }

  return `${clamped.slice(0, 47).trimEnd()}...`;
}
