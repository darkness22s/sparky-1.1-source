...
/**
 * Installs Sparky as a per-user boot service so a connected machine stays
 * reachable through Sparky Cloud after the SSH session ends. Linux-only for
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    "[Unit]",
    "Description=Sparky server (Sparky Cloud)",
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the Sparky background service.";
  }
}
