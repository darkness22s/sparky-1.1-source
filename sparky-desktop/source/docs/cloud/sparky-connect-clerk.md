# Sparky Cloud Clerk Setup

Sparky Cloud uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `t3-relay` template with the shared
`t3-code-relay` audience.

## Application Keys

Sparky Cloud is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:
...
T3CODE_PORT=13773 \
  "/Applications/Sparky (Alpha).app/Contents/MacOS/Sparky (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Sparky (Alpha).app"
codesign -d --entitlements :- "/Applications/Sparky (Alpha).app"
```
...
Approved signed-in users manage Sparky Cloud under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the Sparky Cloud controls on the Connections page.
