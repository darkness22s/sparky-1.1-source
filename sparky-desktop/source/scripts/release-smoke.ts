...
try {
  ...
  // Invoke the checked-in Vite Plus CLI through Node instead of relying on a
  // platform-specific `vp` shim being discoverable by spawnSync. On Windows,
  // Node does not consistently resolve a `.cmd` shim when the caller uses
  // `execFileSync` without `shell: true`; the direct CLI entrypoint is stable
  // in both local and CI environments.
  const vitePlusCliPath = NodePath.resolve(repoRoot, "node_modules/vite-plus/bin/vp");
  assertExists(vitePlusCliPath, "Expected the Vite Plus CLI to be installed.");
  NodeChildProcess.execFileSync(process.execPath, [vitePlusCliPath, "install", "--lockfile-only", "--ignore-scripts"], {
    cwd: tempRoot,
    stdio: "inherit",
  });
  ...
  assertContains(
    nightlyReleaseMetadata,
    "name=Sparky Nightly 9.9.10-nightly.20260413.321 (abcdef123456)",
  ...
  const windowsX64Manifests = NodeFS.readdirSync(releaseAssetsDirectory)
    ...
    .map((name) => NodePath.resolve(releaseAssetsDirectory, name));
