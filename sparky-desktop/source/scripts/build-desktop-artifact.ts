...
// Python PIL-based image resize utility for cross-platform builds.
const PY_RESIZE_SCRIPT = `
import sys
from PIL import Image
src, dst, sz = sys.argv[1], sys.argv[2], int(sys.argv[3])
img = Image.open(src)
img.resize((sz, sz), Image.LANCZOS).save(dst)
`;

function resizeWithPython(sourcePath: string, destPath: string, size: number, verbose: boolean) {
  return runCommand(
    ChildProcess.make("python", ["-c", PY_RESIZE_SCRIPT, sourcePath, destPath, String(size)]),
    { label: `python resize ${size}x${size}`, verbose },
  );
}
...
function generateMacIconSet(
  ...
  return Effect.gen(function* () {
    ...
    // Use sips on macOS, Python PIL on other platforms.
    const trySips = hostPlatform === "darwin";
    ...
    for (const size of iconSizes) {
      ...
      if (trySips) {
        yield* runCommand(
          ChildProcess.make({})`sips -z ${size} ${size} ${sourcePng} --out ${outPath}`,
          { label: `sips icon ${size}x${size}`, verbose },
        );
      } else {
        yield* resizeWithPython(sourcePng, outPath, size, verbose);
      }

      const retinaSize = size * 2;
      const retinaOutPath = path.join(iconsetDir, `icon_${size}x${size}@2x.png`);
      if (trySips) {
        yield* runCommand(
          ChildProcess.make(
            {},
          )`sips -z ${retinaSize} ${retinaSize} ${sourcePng} --out ${retinaOutPath}`,
          { label: `sips icon ${size}x${size}@2x`, verbose },
        );
      } else {
        yield* resizeWithPython(sourcePng, retinaOutPath, retinaSize, verbose);
      }
    }
    ...
    } else if (yield* fs.exists(prebuiltIcns)) {
      yield* fs.copyFile(prebuiltIcns, targetIcns);
      yield* Effect.log(`[desktop-artifact] Using pre-built icns from ${prebuiltIcns}`);
    } else {
      yield* Effect.logWarning(
        "[desktop-artifact] No pre-built .icns found; macOS icon set will be incomplete.",
      );
    }
  });
}
...
function stageMacIcons(
  ...
  return Effect.gen(function* () {
    ...
    const prebuiltIcns = path.join(repoRoot, "assets/prod/sparky-macos.icns");
    ...
    if (hostPlatform === "darwin") {
      ...
      yield* resizeWithPython(sourcePng, iconPngPath, 512, verbose);
    }

    yield* generateMacIconSet(sourcePng, iconIcnsPath, tmpRoot, path, verbose, prebuiltIcns);
  });
}
...
export const resolveDesktopPublishConfig = Effect.fn("resolveDesktopPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    sparkyUpdateUrl: Config.string("SPARKY_UPDATE_URL").pipe(Config.option),
  });
  const sparkyUpdateUrl = Option.getOrUndefined(env.sparkyUpdateUrl)?.trim();
  if (!sparkyUpdateUrl || !/^https:\/\//u.test(sparkyUpdateUrl)) return undefined;
  return {
    provider: "generic" as const,
    url: sparkyUpdateUrl.replace(/\/+$/u, ""),
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Sparky (Nightly)"
    : (desktopPackageJson.productName ?? "Sparky");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    extraResources: [{ from: "apps/desktop/resources/sparky", to: "sparky" }],
  };
...
const assertPlatformBuildResources = Effect.fn("assertPlatformBuildResources")(function* (
  ...
  if (platform === "mac") {
    yield* stageMacIcons(stageResourcesDir, iconAssets.macIconPng, verbose, repoRoot);
    return;
  }
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const directElectronBuilderArgs = [
    electronBuilderCli,
    "--projectDir",
    stageAppDir,
  ...
  const builderArgs = [
    ...
    "--",
    "electron-builder",
    ...directElectronBuilderArgs.slice(1),
  ];
  // electron-builder is a desktop devDependency and is intentionally absent
  // from the staged production install. Prefer the exact host-local CLI when
  // it exists; retain the vp fallback for CI environments that provide the
  // binary through the package manager.
  const builderCommand = (yield* fs.exists(electronBuilderCli))
    ? yield* resolveSpawnCommand(process.execPath, directElectronBuilderArgs, { env: buildEnv })
    : yield* resolveSpawnCommand("vp", builderArgs, { env: buildEnv });
  yield* runCommand(
    ...
    {
      label: `${builderCommand.command} ${builderCommand.args.join(" ")}`,
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for Sparky."),
