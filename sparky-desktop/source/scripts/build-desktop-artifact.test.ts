...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Sparky");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Sparky (Nightly)");
  });
  ...
  it.effect("resolves the Convex-backed desktop publish config from Effect config", () =>
    Effect.gen(function* () {
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                SPARKY_UPDATE_URL: "https://updates.example.test/updates/windows",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveDesktopPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                SPARKY_UPDATE_URL: "https://updates.example.test/updates/macos/arm64",
              },
            }),
          ),
        ),
      ...
      assert.deepStrictEqual(latestConfig, {
        provider: "generic",
        url: "https://updates.example.test/updates/windows",
      });
      assert.deepStrictEqual(nightlyConfig, {
        provider: "generic",
        url: "https://updates.example.test/updates/macos/arm64",
        channel: "nightly",
      });
    }),
  );

  it.effect("fails closed when the Convex update feed is not configured", () =>
    Effect.gen(function* () {
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@sparky/contracts": "workspace:*",
          "@sparky/shared": "workspace:*",
          "@sparky/ssh": "workspace:*",
          "@sparky/tailscale": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.sparky.desktop");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/t3code.provisionprofile");
      assert.deepStrictEqual(mac.protocols, [
        { name: "Sparky", schemes: ["sparky", "sparky-dev"] },
      ]);
  ...
  it.effect("configures a native Debian package for Linux x64 builds", () =>
    Effect.gen(function* () {
      const config = yield* createBuildConfig(
        "linux",
        "deb",
      ...
      assert.equal(config.productName, "Sparky");
