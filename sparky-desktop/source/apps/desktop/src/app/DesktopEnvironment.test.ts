...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/Sparky.app/Contents/Resources/app.asar",
  isPackaged: false,
  resourcesPath: "/Applications/Sparky.app/Contents/Resources",
...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        ...
        environment.path.normalize("/Users/alice/Library/Application Support"),
      );
      assert.equal(environment.baseDir, "/tmp/t3");
      assert.equal(environment.stateDir, environment.path.normalize("/tmp/t3/userdata"));
      assert.equal(
        environment.desktopSettingsPath,
        environment.path.normalize("/tmp/t3/userdata/desktop-settings.json"),
      );
      assert.equal(
        environment.clientSettingsPath,
        environment.path.normalize("/tmp/t3/userdata/client-settings.json"),
      );
      assert.equal(
        environment.savedEnvironmentRegistryPath,
        environment.path.normalize("/tmp/t3/userdata/saved-environments.json"),
      );
      assert.equal(
        environment.serverSettingsPath,
        environment.path.normalize("/tmp/t3/userdata/settings.json"),
      );
      assert.equal(environment.logDir, environment.path.normalize("/tmp/t3/userdata/logs"));
      assert.equal(
        environment.browserArtifactsDir,
        environment.path.normalize("/tmp/t3/userdata/browser-artifacts"),
      );
      ...
      assert.equal(
        ...
        environment.path.resolve("/repo/apps/server/dist/bin.mjs"),
      );
      assert.equal(environment.backendCwd, environment.path.resolve("/repo"));
      assert.equal(environment.appUserModelId, "com.sparky.desktop.dev");
      assert.equal(environment.linuxWmClass, "sparky-dev");
      assert.deepEqual(
  ...
  it.effect("ignores a development server URL in packaged builds", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        { isPackaged: true, appVersion: "1.0.0" },
        { VITE_DEV_SERVER_URL: "http://localhost:5173" },
      );
  ...
  it.effect("resolves picker defaults without nullish sentinels", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(
        ...
        Option.some(environment.path.normalize("/Users/alice/project")),
      );
    }),
  );
});
