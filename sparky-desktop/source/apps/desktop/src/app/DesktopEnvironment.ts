...
const make = Effect.fn("desktop.environment.make")(function* (
  ...
  // A packaged build is always self-contained. Ignore a leaked shell-level
  // VITE_DEV_SERVER_URL so an installed Sparky can never depend on a dev server.
  ...
  const baseDir = Option.getOrElse(configuredBaseDir, () => path.join(homeDirectory, ".sparky"));
  ...
  const userDataDirName = isDevelopment ? "sparky-dev" : "sparky";
  const legacyUserDataDirName = isDevelopment ? "Sparky (Dev)" : "Sparky (Alpha)";
  ...
  return DesktopEnvironment.of({
    ...
    appUserModelId: Option.getOrElse(config.appUserModelIdOverride, () =>
      isDevelopment ? "com.sparky.desktop.dev" : "com.sparky.desktop",
    ),
    linuxDesktopEntryName: isDevelopment ? "sparky-dev.desktop" : "sparky.desktop",
    linuxWmClass: isDevelopment ? "sparky-dev" : "sparky",
    userDataDirName,
