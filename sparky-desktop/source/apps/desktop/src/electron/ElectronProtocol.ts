...
export const DESKTOP_PRODUCTION_SCHEME = "sparky";
export const DESKTOP_DEVELOPMENT_SCHEME = "sparky-dev";
...
export function registerDesktopSchemesAsPrivileged(): void {
  Electron.protocol.registerSchemesAsPrivileged([
    { scheme: DESKTOP_PRODUCTION_SCHEME, privileges: DESKTOP_SCHEME_PRIVILEGES },
    { scheme: DESKTOP_DEVELOPMENT_SCHEME, privileges: DESKTOP_SCHEME_PRIVILEGES },
  ]);
}
...
export interface DesktopProtocolRegistrationInput {
  ...
  readonly neonAuthHostname: string | undefined;
}
...
export function makeDesktopContentSecurityPolicy(input: DesktopProtocolRegistrationInput): string {
  const neonAuthOrigin = input.neonAuthHostname ? `https://${input.neonAuthHostname}` : undefined;
  const scriptSources = [
    "'self'",
    `${input.scheme}:`,
    "'unsafe-inline'",
  ...
  // the build-configured Neon Auth, relay, and OTLP endpoints. Those environment
  ...
  return [
    `default-src 'self' ${input.scheme}:`,
    `script-src ${scriptSources.join(" ")}`,
    `connect-src ${connectSources.join(" ")}`,
    `img-src 'self' ${input.scheme}: blob: data: http: https:`,
    `style-src 'self' ${input.scheme}: 'unsafe-inline'`,
    `font-src 'self' ${input.scheme}: data:`,
    `worker-src 'self' ${input.scheme}: blob:`,
    `frame-src 'self' ${input.scheme}: https://challenges.cloudflare.com`,
    "form-action 'self'",
  ].join("; ");
}
