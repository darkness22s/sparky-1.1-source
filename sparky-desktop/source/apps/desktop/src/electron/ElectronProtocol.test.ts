...
describe("ElectronProtocol", () => {
  ...
  it("registers production and development renderer schemes before app readiness", () => {
    ...
    assert.deepEqual(registerSchemesAsPrivilegedMock.mock.calls, [
      [
        [
          {
            scheme: "sparky",
            privileges: ElectronProtocol.DESKTOP_SCHEME_PRIVILEGES,
          },
          {
            scheme: "sparky-dev",
            privileges: ElectronProtocol.DESKTOP_SCHEME_PRIVILEGES,
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const response = yield* Effect.promise(() =>
            handler!(
              new Request("sparky-dev://app/api/health?verbose=1", {
                headers: {
                  accept: "application/json",
                  origin: "sparky-dev://app",
                  referer: "sparky-dev://app/",
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["default-src"], ["'self'", "t3code:"]);
