...
const AUTO_UPDATE_STARTUP_DELAY = "1 minute";
const AUTO_UPDATE_POLL_INTERVAL = "5 minutes";
...
export const make = Effect.gen(function* () {
  ...
  const startUpdatePollers: Effect.Effect<void, never, Scope.Scope> = Effect.gen(function* () {
    const runCheck = (reason: "startup" | "poll") =>
      checkForUpdates(reason).pipe(
        Effect.catchCause((cause) => {
          if (Cause.hasInterruptsOnly(cause)) {
            return Effect.void;
          }
          const error = new DesktopUpdatePollerError({ poller: reason, cause });
    ...
    yield* Effect.sleep(AUTO_UPDATE_STARTUP_DELAY).pipe(
      Effect.andThen(runCheck("startup")),
      Effect.andThen(
        Effect.sleep(AUTO_UPDATE_POLL_INTERVAL).pipe(
          Effect.andThen(runCheck("poll")),
          Effect.forever,
        ),
      ),
      Effect.catchCause((cause) => {
