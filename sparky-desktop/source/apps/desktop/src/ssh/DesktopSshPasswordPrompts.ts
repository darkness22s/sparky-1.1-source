import type { DesktopSshPasswordPromptRequest } from "@sparky/contracts";
import { DesktopSshPasswordPromptResolutionInputSchema } from "@sparky/contracts";
import type { SshPasswordRequest } from "@sparky/ssh/auth";
...
export class DesktopSshPromptWindowUnavailableError extends Schema.TaggedErrorClass<DesktopSshPromptWindowUnavailableError>()(
  ...
  override get message(): string {
    ...
    return `Sparky window is unavailable during ${this.stage} for SSH authentication to ${this.destination} (request: ${request}).`;
  }
}
