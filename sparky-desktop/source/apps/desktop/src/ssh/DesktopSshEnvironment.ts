...
import * as SshAuth from "@sparky/ssh/auth";
import { discoverSshHosts } from "@sparky/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@sparky/ssh/errors";
import * as SshTunnel from "@sparky/ssh/tunnel";
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Sparky window is not available for SSH authentication.";
      break;
