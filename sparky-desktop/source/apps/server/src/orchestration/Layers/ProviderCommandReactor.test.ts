...
describe("ProviderCommandReactor", () => {
  ...
  it("generates a title from the first message when the client provides no seed", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    harness.generateThreadTitle.mockReturnValue(Effect.succeed({ title: "Generated from text" }));
  ...
  it("restarts a Codex app-server when the selected context window changes", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  it("settles an interrupted turn and records a failure when provider interrupt fails", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        session: {
          ...
          providerName: "codex",
          runtimeMode: "approval-required",
          activeTurnId: asTurnId("turn-interrupt-failure"),
          lastError: null,
          updatedAt: now,
        },
        createdAt: now,
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        threadId: ThreadId.make("thread-1"),
        turnId: asTurnId("turn-interrupt-failure"),
        createdAt: now,
      }),
    );
    ...
    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.session?.lastError).toContain("interrupt unavailable");
  });
  ...
  it("settles a session and records a failure when provider stop fails", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
    expect(thread?.session?.lastError).toContain("stop unavailable");
  });
});
