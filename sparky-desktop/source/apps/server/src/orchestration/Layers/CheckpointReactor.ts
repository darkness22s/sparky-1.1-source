...
const make = Effect.gen(function* () {
  ...
  const captureAndDispatchCheckpoint = Effect.fn("captureAndDispatchCheckpoint")(function* (input: {
    ...
    // Retry checkpoint capture up to 3 times with exponential backoff.
    // If it still fails, log the error and mark the checkpoint as missing
    // rather than crashing the entire turn.
        ...
        Effect.retry({
          while: (error) => {
            ...
            return !(
              ...
              detail.includes("Cannot chdir") ||
              detail.toLowerCase().includes("permission denied")
            );
          },
          times: 3,
          schedule: Schedule.exponential(Duration.seconds(1), 2.0),
        }),
        Effect.as(true),
        Effect.catch((error) =>
          Effect.gen(function* () {
            yield* Effect.logWarning("checkpoint capture failed after retries", {
              threadId: input.threadId,
              turnId: input.turnId,
              cwd: input.cwd,
            ...
            yield* appendCaptureFailureActivity({
              threadId: input.threadId,
              turnId: input.turnId,
              detail: `Checkpoint capture failed: ${error.message}`,
              createdAt: input.createdAt,
            });
            return false;
          }),
  ...
  const ensurePreTurnBaselineFromTurnStart = Effect.fn("ensurePreTurnBaselineFromTurnStart")(
    function* (event: Extract<ProviderRuntimeEvent, { type: "turn.started" }>) {
      ...
      // Retry baseline checkpoint capture. Failure is non-fatal - log and continue.
      const captured = yield* checkpointStore
        .captureCheckpoint({
          cwd: checkpointCwd,
          checkpointRef: baselineCheckpointRef,
        ...
        .pipe(
          ...
          Effect.as(true),
          Effect.catch((error) =>
            Effect.logWarning("baseline checkpoint capture failed", {
              threadId: thread.id,
              turnId: turnId,
              cwd: checkpointCwd,
              detail: error.message,
  ...
  const ensurePreTurnBaselineFromDomainTurnStart = Effect.fn(
    ...
    const captured = yield* checkpointStore
      ...
      .pipe(
        ...
        Effect.as(true),
        Effect.catch((error) =>
          Effect.logWarning("domain baseline checkpoint capture failed", {
            threadId,
            cwd: checkpointCwd,
            detail: error.message,
          }).pipe(Effect.as(false)),
  ...
  const processDomainEvent = Effect.fn("processDomainEvent")(function* (event: OrchestrationEvent) {
    ...
    // replace it. Runtime events are also consumed below through an eager
    // ProviderService subscription so terminal checkpoint events are not lost.
