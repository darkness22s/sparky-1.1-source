...
describe("ProviderRuntimeIngestion", () => {
  ...
  async function createHarness(options?: {
    ...
    await Effect.runPromise(
      engine.dispatch({
        ...
        workspaceRoot,
        defaultModelSelection: options?.modelSelection ?? {
  ...
  it("settles provider turn.aborted events as interrupted turns", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await waitForThread(
      harness.readModel,
      (thread) => thread.session?.activeTurnId === "turn-aborted",
    );
    ...
    harness.emit({
      ...
      turnId: asTurnId("turn-aborted"),
      payload: { reason: "Provider interrupted the turn." },
    });
  ...
  it("does not treat an interrupted turn.completed event as successful", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-turn-completed-interrupted-started"),
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
      createdAt: now,
      turnId: asTurnId("turn-completed-interrupted"),
    });
    await waitForThread(
      harness.readModel,
      (thread) => thread.session?.activeTurnId === "turn-completed-interrupted",
    );
  ...
  it("does not resurrect a settled turn from a late running session event", async () => {
    ...
    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-settled-turn-started"),
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
      createdAt: now,
      turnId: asTurnId("turn-settled"),
    });
    await waitForThread(
      harness.readModel,
      (thread) => thread.session?.activeTurnId === "turn-settled",
    );

    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-settled-turn-completed"),
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
      createdAt: now,
      turnId: asTurnId("turn-settled"),
    ...
    harness.emit({
      ...
      eventId: asEventId("evt-late-running-after-completion"),
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
      createdAt: "2026-01-01T00:00:01.000Z",
  ...
  it("does not overwrite a local interrupt with a late provider completion", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";

    harness.emit({
      type: "turn.started",
      eventId: asEventId("evt-local-interrupt-started"),
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
      createdAt: now,
      turnId: asTurnId("turn-local-interrupt"),
    });
    await waitForThread(
      harness.readModel,
      (thread) => thread.session?.activeTurnId === "turn-local-interrupt",
    );

    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-local-interrupt-settled"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "interrupted",
    ...
    harness.emit({
      type: "turn.completed",
      eventId: asEventId("evt-late-after-local-interrupt"),
      provider: ProviderDriverKind.make("codex"),
      threadId: asThreadId("thread-1"),
  ...
  it("buffers assistant deltas when live streaming is disabled", async () => {
  ...
  it("streams assistant deltas by default when a turn starts", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
  ...
  it("maps session/thread lifecycle and item.started into session/activity projections", async () => {
    ...
    expect(started?.payload).toMatchObject({
      status: "in_progress",
      title: "Read file",
      data: { toolCallId: "call-read-file", toolName: "read" },
    });
  });
  ...
  it("persists an observed provider context window into the same thread selection", async () => {
    const harness = await createHarness();
    const baseEvent = {
    ...
    harness.emit({
      ...baseEvent,
      eventId: asEventId("evt-context-selection-observed-1"),
      createdAt: "2026-01-01T00:00:00.000Z",
    });

    const first = await waitForThread(
      harness.readModel,
      (entry) =>
        entry.modelSelection.options?.some(
          (option) => option.id === "contextWindow" && option.value === "258400",
        ) === true,
    );
    expect(first.modelSelection.contextWindowSource).toBe("provider");

    harness.emit({
      ...baseEvent,
      eventId: asEventId("evt-context-selection-observed-2"),
      createdAt: "2026-01-01T00:00:01.000Z",
    });
    await harness.drain();

    const second = await harness.readModel();
    const contextOptions = second.threads[0]?.modelSelection.options?.filter(
  ...
  it("caps an overreported OAuth Codex usage window in both UI surfaces", async () => {
    ...
    const harness = await createHarness({ modelSelection });

    harness.emit({
      type: "thread.token-usage.updated",
      eventId: asEventId("evt-oauth-context-overreported"),
      provider: ProviderDriverKind.make("codex"),
      createdAt: "2026-01-01T00:00:00.000Z",
      threadId: asThreadId("thread-1"),
      payload: { usage: { usedTokens: 242_000, maxTokens: 1_050_000 } },
    });

    const thread = await waitForThread(
      harness.readModel,
      (entry) =>
        entry.modelSelection.contextWindowSource === "oauth" &&
        entry.modelSelection.options?.some(
          (option) => option.id === "contextWindow" && option.value === "258400",
        ) === true &&
        entry.activities.some(
    ...
    expect(thread.modelSelection.contextWindowSource).toBe("oauth");
    expect(thread.modelSelection.options).toEqual([{ id: "contextWindow", value: "258400" }]);
    const activity = thread.activities.find(
      (entry: ProviderRuntimeTestActivity) => entry.kind === "context-window.updated",
    );
    expect(activity?.payload).toMatchObject({ maxTokens: 258_400 });
  });
