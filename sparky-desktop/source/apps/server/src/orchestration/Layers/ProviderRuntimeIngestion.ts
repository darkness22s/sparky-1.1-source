...
function buildContextWindowActivityPayload(
  ...
  if (
    maxTokens === undefined ||
    (selection?.contextWindowSource !== "oauth" && !isCodexOAuthModel(selection?.model))
  ) {
  ...
  return {
    ...event.payload.usage,
    maxTokens:
      normalizeCodexOAuthEffectiveContextWindowTokens(selection?.model, maxTokens) ?? maxTokens,
  };
}
...
function modelSelectionForObservedContextWindow(
  ...
  const maxTokens = event.payload.usage.maxTokens;
  if (maxTokens === undefined || maxTokens <= 0) return undefined;

  const selection = thread.modelSelection;
  // A provider instance id is authoritative when present. For legacy events
  // without it, only update the default instance whose id is the driver kind;
  // never attach one instance's capacity to another configured instance.
  if (
    (event.providerInstanceId !== undefined && selection.instanceId !== event.providerInstanceId) ||
    (event.providerInstanceId === undefined &&
      String(selection.instanceId) !== String(event.provider))
  ) {
    return undefined;
  }
  ...
  const oauthContext =
    selection.contextWindowSource === "oauth" || isCodexOAuthModel(selection.model);
  const effectiveMaxTokens = oauthContext
    ? (normalizeCodexOAuthEffectiveContextWindowTokens(selection.model, maxTokens) ?? maxTokens)
    : maxTokens;
  const source = oauthContext ? "oauth" : "provider";
  if (
    getModelSelectionContextWindowTokens(selection) === effectiveMaxTokens &&
    selection.contextWindowSource === source
  ) {
    return undefined;
  }
...
const make = Effect.gen(function* () {
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      const pendingTurnStartForSessionState =
        ...
        (event.payload.state === "running" || event.payload.state === "waiting")
          ...
          : Option.none();
      const hasSettledLatestTurn =
        thread.latestTurn !== null && thread.latestTurn.state !== "running";
      const sessionWasSettledBeforeEvent =
        thread.session !== null &&
      ...
      const orphanedRunningSessionState =
        event.type === "session.state.changed" &&
        (event.payload.state === "running" || event.payload.state === "waiting") &&
        activeTurnId === null &&
        Option.isNone(pendingTurnStartForSessionState) &&
        (hasSettledLatestTurn || sessionWasSettledBeforeEvent);
      const lateTerminalAfterLocalControl =
        activeTurnId === null &&
        (thread.session?.status === "interrupted" ||
          thread.session?.status === "stopped" ||
          thread.session?.status === "error") &&
      ...
      const shouldApplyThreadLifecycle = (() => {
        ...
        switch (event.type) {
          ...
          case "session.state.changed":
            // A late running/waiting notification must not resurrect a
            // completed or failed turn. The only valid no-active-turn case is
            // the short window where the projected turn-start request is still
            // pending and the provider has not emitted turn.started yet.
            return !orphanedRunningSessionState;
          default:
            return true;
        }
      })();
          ...
          yield* orchestrationEngine.dispatch({
            ...
            session: {
              ...
              ...(event.providerInstanceId !== undefined
                ...
                : {}),
              runtimeMode: thread.session?.runtimeMode ?? "full-access",
              // A runtime error is terminal for the current turn. Keeping the
              // provider event id here leaves the read model looking as if a
              // turn is still active even though the session is already in
              // the error state, which makes Stop ineffective on the client.
              activeTurnId: null,
      ...
      const activities = runtimeEventToActivities(event, taskTitle, thread.modelSelection);
  ...
  // Keep persistence/lifecycle processing one event at a time. Assistant text
  // is coalesced at the WebSocket boundary, where it cannot delay terminal or
  // tool events. Batching this worker makes `drain` wait on a timer and can
  // leave a terminal event behind a never-flushed batch during shutdown.
  ...
  const start: ProviderRuntimeIngestionShape["start"] = () =>
    Effect.gen(function* () {
      ...
      yield* Effect.forkScoped(
        Stream.runForEach(providerEvents, (event) => worker.enqueue({ source: "runtime", event })),
      );
      yield* Effect.forkScoped(
