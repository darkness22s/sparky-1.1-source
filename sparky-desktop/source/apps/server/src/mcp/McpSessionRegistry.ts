import { ProviderInstanceId, ThreadId } from "@sparky/contracts";
  ...
  const issue: McpSessionRegistryShape["issue"] = Effect.fn("McpSessionRegistry.issue")(
    function* (request) {
      ...
      const scope: McpInvocationContext.McpInvocationScope = {
        ...
        capabilities: new Set(["preview", "image_view"]),
        issuedAt,
        expiresAt,
      };
...
const make = Effect.acquireRelease(
  ...
  (registry) =>
    Effect.sync(() => {
      ...
      // The registry owns the validity of every provider-scoped token. Do
      // not let the process-local session map retain credentials after the
      // registry is rebuilt or the server runtime is torn down.
...
export const issueActiveMcpCredential = (
  ...
  // Refreshing must not revoke a credential held by an in-flight provider
  // turn. Explicit thread/session teardown still revokes every credential.
