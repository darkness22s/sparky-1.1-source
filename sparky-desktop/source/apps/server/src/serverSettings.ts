...
import { type DeepPartial, deepMerge } from "@sparky/shared/Struct";
import { fromJsonStringPretty, fromLenientJson } from "@sparky/shared/schemaJson";
...
export class ServerSettingsService extends Context.Service<
  ServerSettingsService,
  {
    ...
    /** Acquire a change subscription before forking a hot-reload consumer. */
...
function resolveTextGenerationProvider(settings: ServerSettings): ServerSettings {
  ...
  // Sparky is the built-in provider and is not represented in the legacy
  // provider settings map. Keep the default Sparky selection intact instead
  // of falling through to the first legacy provider (typically Codex).
  if (selection.instanceId === "sparky") {
    return settings;
  }
