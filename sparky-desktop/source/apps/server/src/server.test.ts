...
import {
  ...
  type DpopPublicJwk,
} from "@sparky/shared/dpop";
import { RELAY_HEALTH_REQUEST_TYP, RELAY_MINT_REQUEST_TYP } from "@sparky/shared/relayJwt";
...
import { coalesceThreadDetailEvents } from "./ws.ts";
    ...
    const appLayer = servedRoutesLayer.pipe(
      ...
      Layer.provide(
        Layer.mock(CloudCliTokenManager.CloudCliTokenManager)({
          get: Effect.die(new Error("Unexpected Sparky Cloud CLI authorization request.")),
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves the documented Sparky Cloud mint credential endpoint", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
  ...
  it.effect("serves signed Sparky Cloud environment health checks", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
  ...
  it("coalesces adjacent assistant deltas without swallowing the final event", () => {
    const messageId = MessageId.make("assistant-message");
    const makeDelta = (sequence: number, text: string, streaming: boolean) =>
      ({
        sequence,
        eventId: EventId.make(`event-assistant-${sequence}`),
        aggregateKind: "thread",
        aggregateId: defaultThreadId,
    ...
    const coalesced = coalesceThreadDetailEvents([
      makeDelta(1, "a", true),
      makeDelta(2, "b", true),
      makeDelta(3, "ab", false),
    ]);
    const [merged, final] = coalesced;

    assert.equal(coalesced.length, 2);
    assert.equal(merged?.type, "thread.message-sent");
    assert.equal(merged?.type === "thread.message-sent" ? merged.payload.text : undefined, "ab");
    assert.equal(merged?.sequence, 2);
    assert.equal(final?.sequence, 3);
    assert.equal(final?.type, "thread.message-sent");
    assert.equal(
      final?.type === "thread.message-sent" ? final.payload.streaming : undefined,
      false,
    );
  });

  it("does not coalesce assistant deltas across a lifecycle event", () => {
    const messageId = MessageId.make("assistant-message-boundary");
    const makeDelta = (sequence: number, text: string) =>
      ({
        sequence,
        eventId: EventId.make(`event-assistant-boundary-${sequence}`),
        aggregateKind: "thread",
        aggregateId: defaultThreadId,
        occurredAt: `2026-01-01T00:00:0${sequence}.000Z`,
        commandId: null,
    ...
    const lifecycleEvent = {
      sequence: 2,
      eventId: EventId.make("event-assistant-boundary-lifecycle"),
      aggregateKind: "thread",
      aggregateId: defaultThreadId,
    ...
    assert.equal(coalesced[0]?.type, "thread.message-sent");
    assert.equal(
      coalesced[0]?.type === "thread.message-sent" ? coalesced[0].payload.text : undefined,
      "before",
    );
    assert.equal(coalesced[1]?.type, "thread.session-set");
    assert.equal(coalesced[2]?.type, "thread.message-sent");
    assert.equal(
      coalesced[2]?.type === "thread.message-sent" ? coalesced[2].payload.text : undefined,
      "after",
    );
  });
