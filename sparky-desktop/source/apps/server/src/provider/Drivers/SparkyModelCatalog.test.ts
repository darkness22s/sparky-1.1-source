...
describe("discoverSparkyModels", () => {
  it("adds ChatGPT Codex subscription models when the native auth file exists", async () => {
    ...
    try {
      ...
      expect(
        ...
        options: [
          { id: "128k", label: "128k" },
          { id: "320k", label: "320k", isDefault: true },
        ],
        currentValue: "320k",
      });
    } finally {
  ...
  it("fetches and namespaces every configured provider catalog", async () => {
    ...
    expect(
      ...
      label: "Context Window",
      type: "select",
      options: [{ id: "64k", label: "64k", isDefault: true }],
      currentValue: "64k",
    });
  ...
  it("loads nested Anthropic capabilities from the per-model endpoint", async () => {
    ...
    const model = result.models.find((entry) => entry.slug === "anthropic/claude-capabilities-test");
  ...
  it("fetches Google details and enriches incomplete models from Models.dev", async () => {
    ...
    expect(
      result.models.find((model) => model.slug === "anthropic/claude-no-metadata")?.capabilities
    ...
    expect(
      result.models.find((model) => model.slug === "google/gemini-no-metadata")?.capabilities
        ?.optionDescriptors?.find((descriptor) => descriptor.id === "reasoningEffort"),
    ).toMatchObject({
      options: [
        { id: "minimal" },
