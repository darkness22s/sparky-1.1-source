...
function hasCodexAuthentication(environment: NodeJS.ProcessEnv): boolean {
...
type ModelsDevCatalog = Readonly<
...
function parseReasoningEffortEntries(entries: ReadonlyArray<unknown>): ReadonlyArray<RemoteReasoningEffort> {
...
function contextWindowTokens(value: unknown): number | undefined {
  ...
  const unit = match[2] ?? "";
  const multiplier = unit.startsWith("b") ? 1_000_000_000 : unit === "m" ? 1_000_000 : unit === "k" ? 1_000 : 1;
...
function capCodexContextWindows(
  windows: ReadonlyArray<RemoteContextWindow>,
): ReadonlyArray<RemoteContextWindow> {
  if (!windows.some((window) => window.tokens > CODEX_OAUTH_CONTEXT_WINDOW_TOKENS)) {
    return windows;
  }
  return [
    ...windows.filter((window) => window.tokens < CODEX_OAUTH_CONTEXT_WINDOW_TOKENS),
    {
      id: canonicalContextWindowId(CODEX_OAUTH_CONTEXT_WINDOW_TOKENS),
      tokens: CODEX_OAUTH_CONTEXT_WINDOW_TOKENS,
      label: humanizeContextWindow(CODEX_OAUTH_CONTEXT_WINDOW_TOKENS),
    },
  ];
}
...
function remoteModelFromRecord(value: Record<string, unknown>): RemoteModel | undefined {
...
function mergeRemoteModelMetadata(
  ...
  const metadata = remoteModelFromRecord({ ...fallback, id: model.id });
...
function parseOpenAICompatibleModels(payload: Record<string, unknown>): ReadonlyArray<RemoteModel> {
  ...
  return payload.data.flatMap((value) => (isRecord(value) ? remoteModelFromRecord(value) ?? [] : []));
}
...
async function loadOpenAIModels(
  ...
  return parseOpenAICompatibleModels(
...
async function loadCodexModels(
  ...
  return new Promise((resolve, reject) => {
    ...
    const handleLine = (line: string) => {
      ...
      if (message.id === 2) {
        if (isRecord(message.error)) {
          finish(new Error(nonEmptyString(message.error.message) ?? "Codex model discovery failed"));
          return;
        }
    ...
    child.once("error", (cause) => finish(cause instanceof Error ? cause : new Error(String(cause))));
...
export async function discoverSparkyModels(
  ...
  const configured = PROVIDERS.flatMap((provider) => {
    if (!provider.env) {
      return hasCodexAuthentication(environment) ? [{ provider, apiKey: "authenticated" }] : [];
    }
    const apiKey = environment[provider.env]?.trim();
    return apiKey ? [{ provider, apiKey }] : [];
  });

  const modelsDev = configured.some(({ provider }) => provider.prefix !== "openai" && provider.prefix !== "openai-codex")
    ? await loadModelsDevCatalog(fetchImplementation)
    : undefined;
      ...
      try {
        ...
        const models = remoteModels.flatMap((remoteModel): ReadonlyArray<ServerProviderModel> => {
          ...
          const contextWindows =
            provider.prefix === "openai-codex"
              ? capCodexContextWindows(rawContextWindows)
              : rawContextWindows;
            ...
            remoteModel.defaultContextWindowTokens === undefined
              ...
              : provider.prefix === "openai-codex"
                ? Math.min(
                    ...
                    CODEX_OAUTH_CONTEXT_WINDOW_TOKENS,
                  )
                : remoteModel.defaultContextWindowTokens;
          const defaultContextWindowTokens = contextWindows.some(
            (contextWindow) =>
              contextWindow.tokens === requestedDefaultContextWindowTokens,
          )
