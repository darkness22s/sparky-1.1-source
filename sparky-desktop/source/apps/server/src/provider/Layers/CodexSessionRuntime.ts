...
import { normalizeModelSlug } from "@sparky/shared/model";
...
function isContextWindowExceededError(error: unknown): boolean {
  if (!isCodexAppServerRequestError(error)) return false;
  const details = `${error.errorMessage} ${JSON.stringify(error.data ?? "")}`.toLowerCase();
  return (
    details.includes("contextwindowexceeded") ||
    details.includes("context window") ||
    details.includes("context_length_exceeded")
  );
}
...
const CODEX_COMPACTION_TIMEOUT = "2 minutes" as const;
...
export function withCodexCompactionTimeout<A, E, R>(
  providerThreadId: string,
  effect: Effect.Effect<A, E, R>,
  duration: Duration.Input = CODEX_COMPACTION_TIMEOUT,
      ...
      orElse: () =>
        Effect.fail(
          CodexErrors.CodexAppServerRequestError.internalError(
            "Codex App Server did not report thread compaction completion before the timeout.",
...
export function hasConfiguredMcpServer(
  ...
  return (
    ...
    launchArgs?.includes("mcp_servers.") === true
  );
}

export function reloadConfiguredCodexMcpServer(
  client: Pick<CodexClient.CodexAppServerClient["Service"], "request">,
  appServerArgs: ReadonlyArray<string> | undefined,
  launchArgs?: string,
): Effect.Effect<void, CodexErrors.CodexAppServerError> {
  if (!hasConfiguredMcpServer(appServerArgs, launchArgs)) {
    return Effect.void;
  }
  return client.request("config/mcpServer/reload", undefined).pipe(Effect.asVoid);
}

export function withCodexContextCompactionRetry<A>(
  startTurn: () => Effect.Effect<A, CodexErrors.CodexAppServerError>,
  compact: Effect.Effect<void, CodexErrors.CodexAppServerError>,
  reloadMcpCatalog: () => Effect.Effect<void, CodexErrors.CodexAppServerError>,
  onContextWindowExceeded: Effect.Effect<void> = Effect.void,
): Effect.Effect<A, CodexErrors.CodexAppServerError> {
  return startTurn().pipe(
    Effect.catchIf(isContextWindowExceededError, () =>
      Effect.gen(function* () {
        yield* onContextWindowExceeded;
        yield* compact;
        yield* reloadMcpCatalog();
...
export interface CodexSessionRuntimeOptions {
  ...
  /** Selected model metadata, when available, not a product-wide default. */
  readonly contextWindowTokens?: number | undefined;
  readonly contextWindowSource?: "provider" | "oauth" | undefined;
  readonly serviceTier?: CodexServiceTier | undefined;
  readonly resumeCursor?: CodexResumeCursor;
  readonly appServerArgs?: ReadonlyArray<string>;
}

export function resolveCodexRuntimeContextWindowTokens(input: {
  readonly model?: string;
  readonly contextWindowSource?: "provider" | "oauth" | undefined;
  readonly reportedTokens: number;
  readonly environment?: NodeJS.ProcessEnv;
}): number {
  const oauthContext = input.contextWindowSource === "oauth" || isCodexOAuthModel(input.model);
  return oauthContext
    ? (normalizeCodexOAuthEffectiveContextWindowTokens(
        input.model,
...
export interface CodexSessionRuntimeShape {
  ...
  readonly compact?: () => Effect.Effect<ProviderTurnStartResult, CodexSessionRuntimeError>;
...
> {
  ...
  const config = runtimeModeToThreadConfig(input.runtimeMode);
  const isPlanMode = input.interactionMode === "plan";
  ...
  return decodeCodexTurnStartParamsWithCollaborationMode({
    ...
    input: turnInput,
    approvalPolicy: isPlanMode ? "untrusted" : config.approvalPolicy,
...
export const openCodexThread = (input: {
  ...
  // A resume failure is terminal. Starting a fresh provider thread here
  // silently discards the conversation that the desktop asked us to resume.
  // The caller must surface the failure and let the user explicitly recover.
...
> =>
  Effect.gen(function* () {
    ...
    const compactionWaitersRef = yield* Ref.make(new Map<string, Deferred.Deferred<void>>());
    ...
    const initialSession = {
      ...
      ...(options.contextWindowTokens !== undefined
        ? {
            ...
            contextWindowSource: options.contextWindowSource ?? "provider",
          }
        : {}),
        ...
        Effect.gen(function* () {
          if (method === "thread/tokenUsage/updated") {
            const modelContextWindow = (
              params as {
                readonly tokenUsage?: { readonly modelContextWindow?: unknown };
              }
            ...
            if (
              ...
              const effectiveContextWindow = resolveCodexRuntimeContextWindowTokens({
                ...(currentSession.model ?? options.model
                  ? { model: currentSession.model ?? options.model }
                  : {}),
                contextWindowSource:
                  currentSession.contextWindowSource === "oauth"
                    ? "oauth"
                    : currentSession.contextWindowSource === "provider"
                      ? "provider"
                      : undefined,
                reportedTokens: modelContextWindow,
              ...
              yield* updateSession(sessionRef, {
                ...
                contextWindowSource: (
                  currentSession.contextWindowSource === "oauth" ||
                  isCodexOAuthModel(currentSession.model ?? options.model)
                    ? "oauth"
                    : currentSession.contextWindowSource === "provider"
                      ? "provider"
                      : undefined) as "oauth" | "provider" | undefined,
              });
            }
          }
          if (method === "thread/compacted") {
            const providerThreadId = (params as { readonly threadId: string }).threadId;
            const waiter = (yield* Ref.get(compactionWaitersRef)).get(providerThreadId);
            if (waiter) {
    ...
    const compactThreadAndWait = (providerThreadId: string) =>
      Effect.gen(function* () {
        const waiter = yield* Deferred.make<void>();
        yield* Ref.update(compactionWaitersRef, (waiters) => {
          const next = new Map(waiters);
          next.set(providerThreadId, waiter);
          return next;
        });
        yield* Effect.ensuring(
          withCodexCompactionTimeout(
            providerThreadId,
            client
              .request("thread/compact/start", { threadId: providerThreadId })
              .pipe(Effect.andThen(Deferred.await(waiter))),
          ),
          Ref.update(compactionWaitersRef, (waiters) => {
            const next = new Map(waiters);
    ...
    return {
      ...
      compact: () =>
        Effect.gen(function* () {
          ...
          const compactTurnId = yield* randomUUIDv4("provider-event");
          const turnId = TurnId.make(compactTurnId);

          yield* compactThreadAndWait(providerThreadId);
          yield* updateSession(sessionRef, {
            status: "ready",
            activeTurnId: undefined,
          });
          // `/compact` is a local control operation, but the orchestration
          // layer expects every send to have a normal turn lifecycle.
          yield* emitEvent({
            kind: "notification",
            threadId: options.threadId,
            method: "turn/started",
            ...
            payload: {
              threadId: providerThreadId,
              turn: { id: compactTurnId, items: [], status: "inProgress" },
            },
          });
          yield* emitEvent({
            kind: "notification",
            ...
            payload: {
              threadId: providerThreadId,
              turn: { id: compactTurnId, items: [], status: "completed" },
            },
          });
          return {
            threadId: options.threadId,
      ...
      sendTurn: (input) =>
        Effect.gen(function* () {
          const providerThreadId = yield* readProviderThreadId;
          const reloadMcpCatalog = () =>
            reloadConfiguredCodexMcpServer(client, options.appServerArgs, options.launchArgs);
          // A configured MCP server is part of the turn contract. Running
          // after a failed reload silently leaves Codex with a stale or
          // empty tool catalog, which looks like the model ignored tools.
          yield* reloadMcpCatalog();
          ...
          const startTurn = () => client.raw.request("turn/start", params);
          const rawResponse = yield* withCodexContextCompactionRetry(
            startTurn,
            compactThreadAndWait(providerThreadId),
            reloadMcpCatalog,
            Effect.logWarning(
              "Codex context window reached; compacting before retrying the turn.",
              {
                threadId: options.threadId,
              },
            ),
