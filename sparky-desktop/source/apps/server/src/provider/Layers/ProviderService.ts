      ...
      Effect.flatMap((canonicalEvent) => {
        const resumeCursor =
          canonicalEvent.type === "turn.completed"
            ? (canonicalEvent.payload as { readonly resumeCursor?: unknown }).resumeCursor
            : undefined;
        const persistSparkyResumeCursor =
          canonicalEvent.provider === "sparky" &&
          canonicalEvent.type === "turn.completed" &&
          resumeCursor !== undefined
            ? directory
                ...
                .pipe(
                  Effect.catch((error) =>
                    Effect.logWarning("failed to persist Sparky resume cursor", {
                      threadId: canonicalEvent.threadId,
                      cause: error,
                    }),
                  ),
        ...
        }).pipe(Effect.andThen(publishRuntimeEvent(canonicalEvent)), Effect.andThen(persistSparkyResumeCursor));
      }),
    );
