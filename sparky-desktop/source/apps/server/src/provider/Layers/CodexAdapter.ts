...
import { parseCodexContextWindowTokens, resolveCodexLaunchArgs } from "./codexLaunchArgs.ts";
import {
  isCodexOAuthModel,
...
export const makeCodexAdapter = Effect.fn("makeCodexAdapter")(function* (
  ...
  const nativeEventLogWorker = nativeEventLogger
    ? yield* makeDrainableWorker<NativeEventLogItem, never, never>(({ event, threadId }) =>
      ...
      Effect.gen(function* () {
        ...
        const parsedContextWindowTokens =
          ...
          (input.modelSelection.contextWindowSource !== "models.dev" ||
            ...
            ? parseCodexContextWindowTokens(
                getModelSelectionStringOptionValue(input.modelSelection, "contextWindow"),
              )
            : undefined;
        const contextWindowTokens = normalizeCodexOAuthContextWindowTokens(
          selectedModel,
        ...
        const eventFiber = yield* Stream.runForEach(runtime.events, (event) =>
          Effect.gen(function* () {
            ...
            // The log worker is independent of the runtime event queue, so
            // file logging cannot add back-pressure to provider deltas.
            yield* writeNativeEvent(event);
          }),
  ...
  const sendTurn: CodexAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
    ...
    if (input.attachments?.length === 0 && input.input?.trim().toLowerCase() === "/compact") {
  ...
  const writeNativeEvent = Effect.fn("writeNativeEvent")(function* (event: ProviderEvent) {
    if (!nativeEventLogWorker) {
      return;
    }
    yield* nativeEventLogWorker.enqueue({ event, threadId: event.threadId });
  });
  ...
  yield* Effect.acquireRelease(Effect.void, () =>
    stopAll().pipe(
      Effect.andThen(nativeEventLogWorker?.drain ?? Effect.void),
