...
it.layer(OpenCodeAdapterTestLayer)("OpenCodeAdapterLive", (it) => {
  ...
  it.effect("does not translate the generic Plan mode flag into OpenCode's plan agent", () =>
    Effect.gen(function* () {
      const adapter = yield* OpenCodeAdapter;
      const threadId = asThreadId("thread-no-opencode-plan-routing");
      yield* adapter.startSession({
        provider: ProviderDriverKind.make("opencode"),
        threadId,
        runtimeMode: "full-access",
      ...
      NodeAssert.equal("agent" in (runtimeMock.state.promptCalls.at(-1) as object), false);
    }),
  );
