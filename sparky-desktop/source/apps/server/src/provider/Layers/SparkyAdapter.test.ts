...
describe("Sparky session continuity", () => {
  ...
  it("passes the desktop MCP endpoint without putting its bearer token on the command line", () => {
    ...
    expect(args.at(args.indexOf("--mcp-bearer-token-env-var") + 1)).toBe(
      "T3_MCP_BEARER_TOKEN",
    );
  ...
  it("caps stale OAuth Codex context selections at the usable window", () => {
    expect(normalizeSparkyContextWindow("openai-codex/gpt-5.6-sol", "1m")).toBe("320k");
  ...
  it("passes persisted image files and MIME types to the Rust runtime", () => {
    ...
    expect(args.at(args.indexOf("--image-path") + 1)).toBe(
      "C:\\state\\attachments\\image.png",
    );
