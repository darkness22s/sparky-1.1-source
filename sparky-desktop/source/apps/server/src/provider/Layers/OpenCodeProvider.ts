...
import { compareSemverVersions } from "@sparky/shared/semver";
import {
  buildServerProvider,
  nonEmptyTrimmed,
...
function contextWindowOption(
  ...
  const rawLimit = (model as typeof model & { readonly limit?: { readonly context?: unknown } }).limit;
  const tokens = rawLimit?.context;
  ...
  return {
    ...
    options: [
      {
        id,
        label: id.endsWith("m") ? id.replace(/m$/u, "M") : id,
        isDefault: true,
      },
    ],
    currentValue: id,
...
function openCodeCapabilitiesForModel(input: {
  ...
  const variants = (input.model.variants ?? {}) as Record<
    string,
  ...
  const contextWindow = contextWindowOption(input.model);
        ...
        probe: {
          ...
          message:
            openCodeSettings.serverUrl.trim().length > 0
              ? "OpenCode is disabled in Sparky settings. A server URL is configured."
              : "OpenCode is disabled in Sparky settings.",
        },
      });
    }
...
export const checkOpenCodeProviderStatus = Effect.fn("checkOpenCodeProviderStatus")(function* (
  ...
  if (!isExternalServer) {
    ...
    if (!version) {
      return fallback(
        new Error(
          `Unable to determine OpenCode version from \`opencode --version\` output. Sparky requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
        ),
        null,
      );
    }
