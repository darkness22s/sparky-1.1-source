...
import { resolveClaudeApiModelId, resolveClaudeContextWindow } from "./ClaudeProvider.ts";
const decodeClaudeSettings = Schema.decodeSync(ClaudeSettings);

describe("Claude context provenance", () => {
  it("does not turn a Models.dev fallback into a 1M Claude runtime suffix", () => {
    const selection = {
    ...
    assert.equal(resolveClaudeContextWindow(selection), undefined);
    assert.equal(resolveClaudeApiModelId(selection), "claude-opus-4-6");
  });
});
