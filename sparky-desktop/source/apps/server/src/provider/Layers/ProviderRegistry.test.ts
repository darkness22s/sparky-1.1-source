...
import { deepMerge } from "@sparky/shared/Struct";
import { createModelCapabilities } from "@sparky/shared/model";
import { applyServerSettingsPatch } from "@sparky/shared/serverSettings";
...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsModule.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it("keeps a selected context window stable across fallback refreshes", () => {
        const previousContext = selectDescriptor("contextWindow", "Context Window", [
          { id: "250k", label: "250k", isDefault: true },
        ]);
        const refreshedContext = selectDescriptor("contextWindow", "Context Window", [
          ...
          models: [
            {
              ...
              capabilities: createModelCapabilities({ optionDescriptors: [previousContext] }),
            },
          ],
          slashCommands: [],
          skills: [],
          ...
          models: [
            {
              ...
              capabilities: createModelCapabilities({ optionDescriptors: [refreshedContext] }),
            },
          ],
        } satisfies ServerProvider;

        const stable = mergeProviderSnapshot(previousProvider, refreshedProvider).models[0];
        assert.equal(stable?.contextWindowSource, "provider");
        assert.deepStrictEqual(
          stable?.capabilities?.optionDescriptors?.find(
        ...
        const updated = mergeProviderSnapshot(previousProvider, providerUpdated).models[0];
        assert.deepStrictEqual(
          updated?.capabilities?.optionDescriptors?.find(
            (descriptor) => descriptor.id === "contextWindow",
          ),
          refreshedContext,
        );
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.gen(function* () {
              ...
              assert.strictEqual(
                ...
                "Cursor is disabled in Sparky settings.",
              );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Sparky settings.");
        }),
      );
    });
