...
routing.layer("ProviderServiceLive routing", (it) => {
  ...
  it.effect("persists provider-observed context through stop and same-thread recovery", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      const threadId = asThreadId("thread-context-recovery");
      const selectedModel = {
        ...createModelSelection(codexInstanceId, "gpt-5.6-codex", [
      ...
      routing.codex.emit({
        eventId: asEventId("evt-context-window-observed"),
        provider: CODEX_DRIVER,
        threadId,
      ...
      yield* provider.sendTurn({
        threadId,
        input: "continue in the same session",
        attachments: [],
      });
      ...
      if (recoveredInput && typeof recoveredInput === "object") {
        const modelSelection = (recoveredInput as { modelSelection?: typeof selectedModel })
          .modelSelection;
        assert.equal(modelSelection?.model, selectedModel.model);
        assert.equal(modelSelection?.contextWindowSource, "provider");
        assert.deepEqual(modelSelection?.options, [
          { id: "contextWindow", value: "258400" },
        ]);
        assert.deepEqual(
          (recoveredInput as { resumeCursor?: unknown }).resumeCursor,
...
fanout.layer("ProviderServiceLive fanout", (it) => {
  ...
  it.effect("records first provider-event, first-delta, and processing latency stages", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      const threadId = asThreadId("thread-streaming-latency");
      const session = yield* provider.startSession(threadId, {
        provider: ProviderDriverKind.make("codex"),
        providerInstanceId: codexInstanceId,
        threadId,
      ...
      fanout.codex.emit({
        type: "content.delta",
        eventId: asEventId("evt-streaming-first-delta"),
        provider: ProviderDriverKind.make("codex"),
        threadId,
        turnId: asTurnId("turn-streaming-latency"),
        createdAt: "2026-01-01T00:00:00.000Z",
        payload: {
      ...
      const snapshots = yield* Metric.snapshot;
      assert.equal(
        hasMetricSnapshot(snapshots, "t3_provider_first_runtime_event_duration", {
          provider: "codex",
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "t3_provider_first_assistant_delta_duration", {
          provider: "codex",
          eventType: "content.delta",
        }),
        true,
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "t3_provider_runtime_event_processing_duration", {
          provider: "codex",
          eventType: "content.delta",
        }),
        true,
