...
import { OpenCodeSettings } from "@sparky/contracts";
...
it.layer(testLayer)("checkOpenCodeProviderStatus", (it) => {
  ...
  it.effect("emits OpenCode variant defaults so trait picker can resolve a visible selection", () =>
    Effect.gen(function* () {
      ...
      NodeAssert.equal(variantDescriptor.options.some((option) => option.id === "xhigh"), false);
      ...
      const contextDescriptor = model.capabilities?.optionDescriptors?.find(
        (descriptor) => descriptor.id === "contextWindow" && descriptor.type === "select",
      );
      NodeAssert.ok(contextDescriptor && contextDescriptor.type === "select");
      NodeAssert.equal(contextDescriptor.currentValue, "200k");
    }),
  );
