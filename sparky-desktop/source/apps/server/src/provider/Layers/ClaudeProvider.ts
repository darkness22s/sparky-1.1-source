...
export function resolveClaudeContextWindow(
  ...
  // Models.dev is a discovery fallback, not proof that Claude Code/Anthropic
  // accepts the corresponding API model suffix. Keep the fallback visible in
  // the catalog, but let the provider/runtime report the usable capacity.
  if (modelSelection?.contextWindowSource === "models.dev") {
    return undefined;
  }
  ...
  if (!claudeSettings.enabled) {
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "warning",
        auth: { status: "unknown" },
        message: "Claude is disabled in Sparky settings.",
      },
    });
  }
