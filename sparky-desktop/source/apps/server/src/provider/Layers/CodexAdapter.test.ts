...
validationLayer("CodexAdapterLive validation", (it) => {
  ...
  it.effect("does not force a Models.dev fallback context into Codex", () =>
    Effect.gen(function* () {
      validationRuntimeFactory.factory.mockClear();
      const adapter = yield* CodexAdapter;
      ...
      yield* adapter.startSession({
        provider: ProviderDriverKind.make("codex"),
        threadId: asThreadId("thread-models-dev-context"),
        modelSelection: {
      ...
      NodeAssert.equal(validationRuntimeFactory.factory.mock.calls[0]?.[0]?.contextWindowTokens, undefined);
    }),
  );
});
