...
import { EnvironmentRegistry } from "@sparky/client-runtime/connection";
import { request, runStream } from "@sparky/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@sparky/client-runtime/rpc";
import { ManagedRelay } from "@sparky/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Sparky cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Sparky cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
// independently of Sparky Cloud while clients reach the environment out of band.
