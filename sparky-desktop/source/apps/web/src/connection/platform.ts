...
} from "@sparky/client-runtime/platform";
import {
...
import { bootstrapRemoteBearerSession } from "@sparky/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@sparky/client-runtime/environment";
import { managedRelayAccountChanges, managedRelaySessionAtom } from "@sparky/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@sparky/client-runtime/rpc";
import {
...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Sparky Desktop" : "Sparky Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Sparky Cloud session is unavailable.",
          });
        }
        return token;
      }),
