...
}> = [{ value: ProviderDriverKind.make("sparky"), label: "Sparky", available: true }];
...
export interface WorkLogEntry {
  ...
  /** Line counts captured from an edit/write tool input and retained with the activity row. */
  diffStats?: {
    additions: number;
    deletions: number;
  };
...
export function isLatestTurnSettled(
  ...
  // `running` is also the provider-directory status for a warm reusable
  // session between turns. Only an active turn keeps the latest turn open.
...
function mergeDerivedWorkLogEntries(
  ...
  const diffStats = next.diffStats ?? previous.diffStats;
  return {
    ...previous,
    ...next,
    ...(detail ? { detail } : {}),
...
function firstText(record: Record<string, unknown>, keys: ReadonlyArray<string>): string | null {
...
function diffStatsFromRecord(
  record: Record<string, unknown>,
): { additions: number; deletions: number } | null {
  const numericAdditions = asNumber(record.additions) ?? asNumber(record.addedLines);
  const numericDeletions = asNumber(record.deletions) ?? asNumber(record.removedLines);
  if (numericAdditions !== null || numericDeletions !== null) {
    return {
      additions: Math.max(0, Math.round(numericAdditions ?? 0)),
      deletions: Math.max(0, Math.round(numericDeletions ?? 0)),
    };
  }

  const oldText = firstText(record, ["oldText", "old_text", "before", "original"]);
  const newText = firstText(record, ["newText", "new_text", "after", "replacement", "content"]);
  if (oldText !== null || newText !== null) {
    return {
      additions: lineCount(newText) ?? 0,
      deletions: lineCount(oldText) ?? 0,
    };
  }

  const patch = firstText(record, ["patch", "diff"]);
  if (patch !== null) {
    let additions = 0;
    let deletions = 0;
...
function extractToolDiffStats(payload: Record<string, unknown> | null): WorkLogEntry["diffStats"] {
  if (payload?.itemType !== "file_change") return undefined;
  const data = asRecord(payload.data);
  const rawInput = asRecord(data?.rawInput);
  const item = asRecord(data?.item);
  const kind = asTrimmedString(data?.kind)?.toLowerCase();
  const candidates = [rawInput, item].filter(
    (value): value is Record<string, unknown> => value !== null,
  );

  for (const candidate of candidates) {
    const direct = diffStatsFromRecord(candidate);
    if (direct) return direct;
    for (const key of ["changes", "edits", "operations"]) {
      const nested = candidate[key];
      if (!Array.isArray(nested)) continue;
      const stats = nested.reduce(
        (total, entry) => {
          const record = asRecord(entry);
          const next = record ? diffStatsFromRecord(record) : null;
          return next
            ? {
    ...
    if (kind === "write") {
      const content = firstText(candidate, ["content", "text", "newText", "new_text"]);
      if (content !== null) return { additions: lineCount(content) ?? 0, deletions: 0 };
    }
  }
  return undefined;
}
...
export function derivePhase(session: ThreadSession | null): SessionPhase {
  ...
  // A provider session remains `running` while it is warm and reusable, even
  // between turns.  The task state is represented by activeTurnId; without
  // it the UI must not keep showing the Working/Stop action after completion.
