...
import { createModelSelection, normalizeModelSlug } from "@sparky/shared/model";
...
import { UnifiedSettings } from "@sparky/contracts/settings";
...
export interface ComposerThreadDraftState {
   ...
   * historical `ProviderDriverKind` literals also satisfy the
   * `ProviderInstanceId` slug pattern, so
...
function normalizeProviderModelOptions(
  ...
  const result: ProviderOptionSelectionsByProvider = {};
  for (const providerKey of ["codex", "claudeAgent", "cursor"] as const) {
...
function normalizeModelSelection(
  ...
  const driverKindHint =
    ...
    ProviderDriverKind.make("sparky");
...
export function deriveEffectiveComposerModelState(input: {
  ...
  const isWebsiteDemoRenderer =
    typeof window !== "undefined" &&
    window.location.hostname === "127.0.0.1" &&
    window.location.port === "13823";
  ...
  const selectedModel =
    isWebsiteDemoRenderer && input.threadModelSelection?.model
