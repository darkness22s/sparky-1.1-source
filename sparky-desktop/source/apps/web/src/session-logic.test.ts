...
function makeSession(
  overrides: Pick<ThreadSession, "status" | "activeTurnId">,
): ThreadSession {
  return {
...
describe("deriveWorkLogEntries", () => {
  it("shows a running tool immediately and collapses it into its completion", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "tool-complete",
        createdAt: "2026-02-23T00:00:03.000Z",
        ...
        payload: {
          ...
          status: "completed",
          title: "Terminal",
          data: { toolCallId: "call-1", toolName: "bash", command: "long-running" },
        },
      }),
      makeActivity({
        id: "tool-start",
    ...
    expect(entries[0]?.command).toBe("long-running");
  });

  it("retains a started file edit with live line counts before completion", () => {
      ...
      makeActivity({
        ...
        payload: {
          ...
          data: {
            ...
            toolName: "edit",
            kind: "edit",
            rawInput: { path: "src/app.ts", oldText: "a\nb\n", newText: "c\n" },
          },
        },
      }),
    ]);
  ...
  it("retains edit and write line counts on completed file-change rows", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "edit-tool",
        kind: "tool.completed",
        ...
        payload: {
          ...
          data: {
            kind: "edit",
            rawInput: { path: "src/app.ts", oldText: "old one\nold two\n", newText: "new one\n" },
          },
        },
      }),
      makeActivity({
    ...
    expect(deriveWorkLogEntries(activities).map((entry) => entry.diffStats)).toEqual([
...
describe("isLatestTurnSettled", () => {
  ...
  it("treats an omitted active turn id as a warm, settled session", () => {
    const sessionWithoutActiveTurn = makeSession({
      status: "running",
