...
import { serializeComposerFileLink } from "@sparky/shared/composerTrigger";
...
import type { UnifiedSettings } from "@sparky/contracts/settings";
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const selectedInstanceId = useMemo<ProviderInstanceId>(() => {
    ...
    if (explicitSelectedInstanceId) {
      const sparkyFallback = providerInstanceEntries.find(
        (entry) => entry.enabled && entry.driverKind === "sparky",
      );
      if (sparkyFallback) return sparkyFallback.instanceId;
    ...
    return (
      ...
      activeThreadModelSelection?.instanceId ??
      activeProjectDefaultModelSelection?.instanceId ??
      ProviderInstanceId.make("sparky")
    );
  }, [
    activeProjectDefaultModelSelection?.instanceId,
    activeThread?.session?.providerInstanceId,
