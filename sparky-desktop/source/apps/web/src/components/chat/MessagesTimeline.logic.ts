...
import { type MessageId, type OrchestrationLatestTurn, type TurnId } from "@sparky/contracts";
...
/** The final assistant item is a reliable UI completion signal when the
 * provider's session/turn event is still crossing the websocket boundary. */
export function hasCompletedAssistantTail(
  timelineEntries: ReadonlyArray<TimelineEntry>,
  turnId: TurnId | null,
): boolean {
  if (turnId === null) return false;
  let lastTurnEntry: TimelineEntry | null = null;
  let hasTurnWork = false;
    ...
    const entryTurnId =
      entry.kind === "message" && entry.message.role === "assistant"
        ...
        : entry.kind === "work"
          ? entry.entry.turnId
          : null;
    if (entryTurnId === turnId) {
      lastTurnEntry = entry;
      if (entry.kind === "work") hasTurnWork = true;
  ...
  return (
    lastTurnEntry?.kind === "message" &&
    lastTurnEntry.message.role === "assistant" &&
    !lastTurnEntry.message.streaming &&
    (hasTurnWork || assistantMessageCount > 1)
  );
}
