...
import { resolveSelectableModel } from "@sparky/shared/model";
...
const SPARKY_MODEL_CATALOGS: ReadonlyArray<ModelCatalogSection> = [
...
export const ModelPickerContent = memo(function ModelPickerContent(props: {
  ...
  const isSparkyRuntime = instanceEntries.some((entry) => entry.driverKind === "sparky");
  const [selectedInstanceId, setSelectedInstanceId] = useState<ModelPickerRailSelection>(() => {
    if (props.lockedProvider !== null) {
      ...
      return isSparkyRuntime ? catalogSectionForModel(props.model) : props.activeInstanceId;
    }
    return favorites.length > 0
      ? "favorites"
      : isSparkyRuntime
        ? catalogSectionForModel(props.model)
  ...
  const sidebarInstanceEntries = useMemo(() => {
    const enabledEntries = instanceEntries
      .filter(isProviderInstancePickerVisible)
      .filter((entry) => entry.driverKind !== "sparky");
    if (!isLocked) {
  ...
  const catalogSections = isSparkyRuntime ? SPARKY_MODEL_CATALOGS : [];
  const showSidebar =
    !isSearching && (sidebarInstanceEntries.length > 0 || catalogSections.length > 0);
  ...
  const filteredModels = useMemo(() => {
    ...
    if (props.lockedProvider !== null) {
      ...
      } else if (selectedInstanceId.startsWith("catalog:")) {
        const section = SPARKY_MODEL_CATALOGS.find((item) => item.id === selectedInstanceId);
        result = section
          ? result.filter((m) => isModelInSparkyCatalog(section.id, m.subProvider))
          : [];
      } else {
