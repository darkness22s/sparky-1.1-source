...
export const ProviderModelPicker = memo(function ProviderModelPicker(props: {
  ...
  const isSliderStyle = props.selectorStyle === "slider";
  const displayedTriggerTitle =
    isSliderStyle && props.sliderValueLabel
      ? `${triggerTitle} ${props.sliderValueLabel}`
      : triggerTitle;
  const displayedTriggerLabel =
    isSliderStyle && props.sliderValueLabel
      ? `${triggerLabel} · ${props.sliderValueLabel}`
      : triggerLabel;
  const triggerButton = (
    <Button
      size="sm"
  ...
  if (isSliderStyle) {
    return (
      <div
        ...
        data-fast-mode-supported={props.fastModeSupported ? "true" : "false"}
        data-fast-mode-enabled={props.fastModeEnabled ? "true" : "false"}
      >
        {props.fastModeTriggerControl ?? <UnsupportedFastModeControl placement="trigger" />}
        <Popover open={isMenuOpen} onOpenChange={handleOpenChange}>
          ...
          >
            <div className="mb-2 flex items-center justify-between gap-2">
              ...
              {props.fastModePopupControl ?? <UnsupportedFastModeControl placement="popup" />}
            </div>
