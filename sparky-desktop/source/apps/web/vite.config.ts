...
const configuredMcpServiceUrl = repoEnv.VITE_MCP_SERVICE_URL?.trim() || "";
...
export default defineConfig(({ mode }) => {
  const demoBuild = mode === "demo";
  return {
    base: demoBuild ? "/demo/" : "/",
    plugins: [
      ...(demoBuild ? [] : [tanstackRouter()]),
      react(),
      babel({
    ...
    define: {
      ...
      "import.meta.env.VITE_MCP_SERVICE_URL": JSON.stringify(configuredMcpServiceUrl),
    ...
    build: {
      outDir: demoBuild ? "dist-demo" : "dist",
      emptyOutDir: true,
      sourcemap: demoBuild ? false : buildSourcemap,
      ...(demoBuild
        ? {
            rollupOptions: {
              input: {
